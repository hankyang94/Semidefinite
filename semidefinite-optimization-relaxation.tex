...
\]
We can first form the characteristic polynomial of \(X(z)\) --whose coefficients will be functions of \(z\)-- and then invoking (5) to obtain a finite number scalar inequality constraints. We can then pass these scalar constraints to Mathematica and plot the set as in the following figure.
...
\caption{An example spectrahedron.}\label{fig:spectrahedron-stride}
\end{figure}
...
\begin{equation}
u^\top C_{\alpha} u = (1+\alpha) u^\top A u - \alpha u^\top B u = (1+\alpha) u^\top A u \left(  1- \frac{\alpha}{1+\alpha} \frac{u^\top B u}{u^\top A u}  \right).
